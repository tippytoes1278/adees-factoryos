...
// Stage 3 (8.B3): one-time DEV backfill of RECEIVED_BREAKDOWN for the cards that
// drifted while receives could omit a per-size breakdown (pre-8.B2). The three
// approved corrections are hard-coded in STAGE3_DEV_WRITES; each is verified
// against the card's live state before writing and skipped with a reason if the
// sheet has moved on (e.g. PAIRS_RECEIVED no longer matches the approved total).
// DEV ONLY: opens CONFIG.DEV_SHEET_ID explicitly — LIVE is untouchable by
// construction, regardless of CONFIG.ENV. Idempotent (already-consistent cards
// are skipped). Run manually: Run ▸ runStage3DevBackfill.
var STAGE3_DEV_WRITES = [
  { jobCardId:'JC-2026-012', write:{ 'UK 6':5, 'UK 7':5, 'UK 8':5, 'UK 9':5, 'UK 10':5, 'UK 11':5 } },
  { jobCardId:'JC-2026-013', write:{ 'UK 6':5, 'UK 7':5, 'UK 8':5, 'UK 9':5, 'UK 10':5, 'UK 11':5 } },
  { jobCardId:'JC-2026-014', write:{ 'UK 6':5, 'UK 7':5, 'UK 8':5, 'UK 9':5, 'UK 10':5, 'UK 11':5 } }
];

function runStage3DevBackfill() {
  var lock = LockService.getPublicLock();
  try {
    lock.waitLock(10000);
    var devSs = SpreadsheetApp.openById(CONFIG.DEV_SHEET_ID);   // never the ENV sheet
    var ws = ensureJobCardsSheet(devSs);
    var lastRow = ws.getLastRow();
    if (lastRow < 2) return { success:false, error:'No job cards on DEV sheet.' };
    var rows = ws.getRange(2, 1, lastRow-1, 18).getValues();
    var byId = {};
    rows.forEach(function(r, i){ byId[safeStr(r[0]).trim()] = { r:r, sheetRow:i+2 }; });

    var details = [], applied = 0;
    STAGE3_DEV_WRITES.forEach(function(w) {
      var hit = byId[w.jobCardId];
      if (!hit) { details.push({ jobCardId:w.jobCardId, action:'skipped', reason:'card not found' }); return; }
      var r = hit.r;
      var pairsReceived = safeNum(r[7]);                                   // col H = PAIRS_RECEIVED
      var issuedSb = {}; try { issuedSb = JSON.parse(safeStr(r[8])) || {}; } catch(e) {}
      var currentRb = {}; try { currentRb = JSON.parse(safeStr(r[17])) || {}; } catch(e) {}

      // Guards: the approved write must total the card's CURRENT pairs received
      // and stay within issued per size; skip if the ledger is already consistent.
      var curSum = 0; Object.keys(currentRb).forEach(function(k){ curSum += safeNum(currentRb[k]); });
      if (curSum === pairsReceived) { details.push({ jobCardId:w.jobCardId, action:'skipped', reason:'already consistent (' + curSum + '=' + pairsReceived + ')' }); return; }
      var wSum = 0, bad = '';
      Object.keys(w.write).forEach(function(k){ var q = safeNum(w.write[k]); wSum += q; if (q > safeNum(issuedSb[k])) bad = k; });
      if (wSum !== pairsReceived) { details.push({ jobCardId:w.jobCardId, action:'skipped', reason:'approved write totals ' + wSum + ' but PAIRS_RECEIVED is ' + pairsReceived + ' — card moved since approval, re-table' }); return; }
      if (bad) { details.push({ jobCardId:w.jobCardId, action:'skipped', reason:'size "' + bad + '" exceeds issued' }); return; }

      ws.getRange(hit.sheetRow, 18).setValue(JSON.stringify(w.write));   // col R = RECEIVED_BREAKDOWN
      applied++;
      details.push({ jobCardId:w.jobCardId, action:'applied', was:currentRb, now:w.write });
    });

    SpreadsheetApp.flush();
    ['dashboardData_DEV','storeScreenData_DEV'].forEach(function(k){ try { CacheService.getScriptCache().remove(k); } catch(e) {} });
    Logger.log('runStage3DevBackfill: applied ' + applied + '/' + STAGE3_DEV_WRITES.length + ' — ' + JSON.stringify(details));
    return { success:true, applied:applied, details:details };
  } catch(e) {
    return { success:false, error:e.message };
  } finally {
    lock.releaseLock();
